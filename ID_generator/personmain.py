...
class DiffusionTrainerLite(LightningLite):
    ...
    def create_and_save_sample_grid(self, model, size, x=None, context=None, latent_decoder=None, save_path=''):
        ...
        model.eval()
        new_size = Resize([512, 200]) 
        with torch.no_grad():


            if context is not None:

                # add create synthetic random samples block and add them to the grid
                ...
                grid = make_grid(samples_syn_cond, nrow=N_PER_ROW, pad_value=PAD_VALUE, padding=PADDING)
                ...
                samples_cond = new_size(samples_cond) 
            ...
            if x is not None:
                grid = torch.cat(
                    [grid, divider, make_grid(new_size(x[:N_PER_BLOCK].cpu()), nrow=N_PER_ROW, pad_value=PAD_VALUE, padding=PADDING)], dim=1) 
    ...
    def run(self, cfg):
        ...
        while True:
            for x, context in dataloader:
                ...
                log_file_path = os.path.join(os.getcwd(), "personmain.log") 
